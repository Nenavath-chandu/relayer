...
# ──────────────────────────────────────────────────────────────────────────────
# ENDPOINT 3 — Studio Generation (Images via Gemini Imagen-3)
# ──────────────────────────────────────────────────────────────────────────────
import base64

@app.post("/generate")
def generate_studio(prompt: str = Form(...)):
    """
    Generates a highly realistic AI image based on the prompt using Imagen-3.
    Returns: JSON with base64 encoded image data.
    """
    if not GEMINI_KEY:
        return {"status": "error", "message": "Add GEMINI_API_KEY to backend/.env"}
    
    try:
        # Enhance prompt for high-quality Reality Layer output
        enhanced_prompt = f"Ultra-realistic photography, 8k resolution, highly detailed, cinematic lighting. {prompt}"
        ...
        if result.generated_images:
            ...
            return {"status": "ok", "image_base64": f"data:image/jpeg;base64,{b64_str}"}
        else:
            return {"status": "error", "message": "No image generated by AI."}
